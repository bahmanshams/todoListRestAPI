...
@csrf_exempt
def all_todo_one_view(request):
    try:
        all_todo = Todo.objects.filter(id=request.POST.get('id'))
...
@csrf_exempt
def all_subset_one_view(request):
    try:
        all_subset = Subset.objects.filter(id=request.POST.get('id'))
        all_subset_serialized = serializers.serialize('json', all_subset)
        all_subset_json = json.loads(all_subset_serialized)
        data = json.dumps(all_subset_json)
        return HttpResponse(data)
    except:
        return HttpResponse("Not Ok")
...
@csrf_exempt
def all_cat_one_view(request):
    try:
        all_cat = Category.objects.filter(id=request.POST.get('id'))
...
@csrf_exempt
def update_todo(request, done=False):
    try:
        id = request.POST.get('id')
        description = request.POST.get('description')
        avatar = request.POST.get('avatar')
